...
entities_unicodes_manual_titlecase_special_c() {
	...
	if (
		${5} && (
			hestiaUNICODES_locale_is(${5}, HestiaUNICODES_To_CString(\"az\")) ||
			hestiaUNICODES_locale_is(${5}, HestiaUNICODES_To_CString(\"tr\"))
		)
	) {
		// Azerbaijani & Turkish
		switch (${2}) {
		...
		${5} &&
		hestiaUNICODES_locale_is(${5}, HestiaUNICODES_To_CString(\"lt\"))
	) {
		// Lithuanian
		switch (${2}) {
		case 0x0049:
