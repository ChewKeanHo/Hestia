...
views_unicodes_write_special_content_titlecase_nim() {
        ...
        # execute
        printf -- "%s" "\
$(views_get_indent_nim "$(( $4 + 1 ))")cut_count = ${5}
$(views_get_indent_nim "$(( $4 + 1 ))")if ${6} == true:
" >> "${1}.tmp"
        if [ $? -ne 0 ]; then
                return 1
        fi
        ...
        while [ ! "$____buffer" = "" ]; do
                ...
                printf -- "%s" "\
$(views_get_indent_nim "$(( $4 + 2 ))")output_char[${____index}] = ${____char}
" >> "${1}.tmp"
                if [ $? -ne 0 ]; then
                        return 1
                fi
...
" >> "${1}.tmp"
        ...
        printf -- "%s" "\
$(views_get_indent_nim "$(( $4 + 1 ))")return 0
" >> "${1}.tmp"
        if [ $? -ne 0 ]; then
                return 1
        fi
