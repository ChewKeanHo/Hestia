...
"
        ...
        ## all good - print now
        printf -- "%s" "\
$(views_get_indent_go "$(( $4 + 1 ))")if ${6} == \"\" {
$(views_get_indent_go "$(( $4 + 2 ))")*cut_count = ${5}
$(views_get_indent_go "$(( $4 + 2 ))")*output_char = []Rune{ ${____output_data_titlecase} }
$(views_get_indent_go "$(( $4 + 2 ))")return 0
$(views_get_indent_go "$(( $4 + 1 ))")} else {
$(views_get_indent_go "$(( $4 + 2 ))")*cut_count = ${5}
$(views_get_indent_go "$(( $4 + 2 ))")*output_char = []Rune{ ${____output_data_lowercase} }
$(views_get_indent_go "$(( $4 + 2 ))")return 0
$(views_get_indent_go "$(( $4 + 1 ))")}
" >> "${1}.tmp"
        if [ $? -ne 0 ]; then
