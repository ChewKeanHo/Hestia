...
interactors_unicodes_casing_write_content_special_start_powershell() {
        ...
        "$UNICODES_OUTPUT_TYPE_TITLECASE")
                interactors_unicodes_casing_write_content_special_start_titlecase_powershell \
                        "$1"
                if [ $? -ne 0 ]; then
                        return 1
                fi
        ...
        "$UNICODES_OUTPUT_TYPE_UPPERCASE")
                interactors_unicodes_casing_write_content_special_start_uppercase_powershell \
                        "$1"
                if [ $? -ne 0 ]; then
                        return 1
                fi
