...
interactors_unicodes_casing_write_content_special_value_titlecase_rust() {
        #____dest_path="$1"
        #____codepoints="$2"
        #____indent_level="$3"
        #____trim_count="$4"
...
$(views_get_indent_rust "$(( $3 + 1 ))")if ${UNICODES_ELEMENT_NAME_TO_TITLECASE} == true {
$(views_get_indent_rust "$(( $3 + 2 ))")*cut_count = ${4};
$(views_get_indent_rust "$(( $3 + 2 ))")*output_char = vec![${____output_data_titlecase}];
$(views_get_indent_rust "$(( $3 + 2 ))")return 0;
$(views_get_indent_rust "$(( $3 + 1 ))")} else {
$(views_get_indent_rust "$(( $3 + 2 ))")*cut_count = ${4};
$(views_get_indent_rust "$(( $3 + 2 ))")*output_char = vec![${____output_data_lowercase}];
$(views_get_indent_rust "$(( $3 + 2 ))")return 0;
$(views_get_indent_rust "$(( $3 + 1 ))")}
"
        if [ $? -ne 0 ]; then
