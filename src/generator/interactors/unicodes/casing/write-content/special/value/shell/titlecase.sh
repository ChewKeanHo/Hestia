...
$(views_get_indent_shell "$(( $3 + 1 ))")if [ ! \"\$${UNICODES_ELEMENT_NAME_TO_TITLECASE}\" = \"\" ]; then
$(views_get_indent_shell "$(( $3 + 2 ))")printf -- \"%s\" \"\\
${4}
${____output_data_titlecase}
\"
$(views_get_indent_shell "$(( $3 + 2 ))")return 0
$(views_get_indent_shell "$(( $3 + 1 ))")else
$(views_get_indent_shell "$(( $3 + 2 ))")printf -- \"%s\" \"\\
${4}
${____output_data_lowercase}
\"
$(views_get_indent_shell "$(( $3 + 2 ))")return 0
$(views_get_indent_shell "$(( $3 + 1 ))")fi
"
        if [ $? -ne 0 ]; then
