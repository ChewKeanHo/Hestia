...
$(views_get_indent_python "$(( $3 + 4 ))")return (${4}, [${____output_data}])
"
                ;;
        2)
                views_write_raw_content_python "$1" "\
$(views_get_indent_python "$(( $3 + 3 ))")return (${4}, [${____output_data}])
"
                ;;
        *)
                views_write_raw_content_python "$1" "\
$(views_get_indent_python "$(( $3 + 2 ))")return (${4}, [${____output_data}])
"
                ;;
        esac
        if [ $? -ne 0 ]; then
