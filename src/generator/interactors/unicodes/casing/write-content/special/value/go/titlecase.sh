...
$(views_get_indent_go "$(( $3 + 1 ))")*cut_count = ${4}
$(views_get_indent_go "$(( $3 + 1 ))")if ${UNICODES_ELEMENT_NAME_TO_TITLECASE} == true {
$(views_get_indent_go "$(( $3 + 2 ))")*output_char = []Rune{ ${____output_data_titlecase} }
$(views_get_indent_go "$(( $3 + 1 ))")} else {
$(views_get_indent_go "$(( $3 + 2 ))")*output_char = []Rune{ ${____output_data_lowercase} }
$(views_get_indent_go "$(( $3 + 1 ))")}
$(views_get_indent_go "$(( $3 + 1 ))")return 0
"
        if [ $? -ne 0 ]; then
