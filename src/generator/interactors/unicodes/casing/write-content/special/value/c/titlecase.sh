...
$(views_get_indent_c "$(( $3 + 1 ))")if (${UNICODES_ELEMENT_NAME_TO_TITLECASE} == HestiaBOOLEANS_TRUE) {
"
        if [ $? -ne 0 ]; then
                return 1
        fi
        ...
        while [ ! "$____buffer" = "" ]; do
                ...
                views_write_raw_content_c "$1" "\
$(views_get_indent_c "$(( $3 + 2 ))")(*output_char)[${____index}] = ${____char};
"
                if [ $? -ne 0 ]; then
