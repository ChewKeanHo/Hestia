...
. "${PROJECT_PATH_ROOT}/presenters/locales/languages/name/write-content/writer/main.sh"
if [ $? -ne 0 ]; then
        return 1
fi
...
while IFS="" read -r LOCALES_LINE || [ -n "$LOCALES_LINE" ]; do
        ...
        [ ! "$LOCALES_OFFICIAL_CODE" = "${LOCALES_CODE%%-*}" ]; then
                ...
                [ ! "$LOCALES_OFFICIAL_CODE" = "$LOCALES_PREVIOUS_CODE" ]; then
                        HestiaOS_Print_Info "\
Writing '\"${LOCALES_OFFICIAL_CODE}\" = \"${LOCALES_OFFICIAL_VALUE}\" # ${LOCALES_OFFICIAL_LABEL}' Locales Languages' Name Entry...
"
                ...
                # reset official languages/nameuage for new entry
...
"
        ...
        # all good - write now
        HestiaOS_Print_Info "\
Writing '${LOCALES_LINE}' Locales Languages' Name Entry...
"
        presenters_locales_languages_name_write_content \
                "$LOCALES_CODE" \
                "$LOCALES_VALUE" \
